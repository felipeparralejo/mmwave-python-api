...
CONFIG_STATUS = '0000'
CONFIG_FOOTER = 'aaee'
ADC_PARAMS = {'chirps': PARAMS.CHIRP_LOOPS,
...
class DCA1000:
    ...
    def configure(self):
        ...
        print(self._send_command(
            CMD.CONFIG_FPGA_GEN_CMD_CODE, '0600', 'c005350c0000'))

        # CONFIG_PACKET_DATA_CMD_CODE
        # 5a a5 0b 00 06 00 c0 05 35 0c 00 00 aa ee
        print(self._send_command(
            CMD.CONFIG_PACKET_DATA_CMD_CODE, '0600', 'c005350c0000'))
    ...
    def read(self, timeout=1):
        ...
        while True:
            ...
            try:
                ret_frame[curr_idx *
                          UINT16_IN_PACKET:(curr_idx + 1) * UINT16_IN_PACKET] = packet_data
            except:
                pass
    ...
    def _send_command(self, cmd, length='0000', body='', timeout=1):
        ...
        resp = ''
        msg = codecs.decode(
            ''.join((CONFIG_HEADER, str(cmd), length, body, CONFIG_FOOTER)), 'hex')
        try:
    ...
    def _clear_buffer(self):
        ...
        while True:
            try:
                _, _ = self.data_socket.recvfrom(1500)
            except IOError:
                break
