...
visTrigger = plot2DscatterXY + plot2DscatterXZ + \
    plot3Dscatter + plotRangeDopp + plotCustomPlt
...
if loadData:
    adc_data = pickle.load(open('openradar_data_test.bin', 'rb'))
else:
    adc_data = dca.read()
frame = dca.organize(adc_data, num_chirps=numChirpsPerFrame,
                     num_rx=numRxAntennas, num_samples=numADCSamples)

# (2) Range Processing
radar_cube = dsp.range_processing(
    frame, window_type_1d=Window.BLACKMAN)
...
det_matrix, aoa_input = dsp.doppler_processing(
    radar_cube, num_tx_antennas=3, clutter_removal_enabled=True)
...
snr = peakVals - \
    noiseFloorRange[det_peaks_indices[:, 0], det_peaks_indices[:, 1]]

dtype_location = '(' + str(numTxAntennas) + ',)<f4'
dtype_detObj2D = np.dtype({'names': ['rangeIdx', 'dopplerIdx', 'peakVal', 'location', 'SNR'],
                           'formats': ['<i4', '<i4', '<f4', dtype_location, '<f4']})
detObj2DRaw = np.zeros(
    (det_peaks_indices.shape[0],), dtype=dtype_detObj2D)
...
detObj2DRaw = dsp.prune_to_peaks(
    detObj2DRaw, det_matrix, numDopplerBins, reserve_neighbor=True)

# --- Peak Grouping
detObj2D = dsp.peak_grouping_along_doppler(
    detObj2DRaw, det_matrix, numDopplerBins)
...
detObj2D = dsp.range_based_pruning(
    detObj2D, SNRThresholds2, peakValThresholds2, numRangeBins, 0.5, range_resolution)
...
for i, currRange in enumerate(Ranges):
    if i >= (detObj2D_f.shape[0]):
        # copy last row
        detObj2D_f = np.insert(
            detObj2D_f, i, detObj2D_f[i - 1], axis=0)
...
cluster = clu.radar_dbscan(
    detObj2D_f, 0, doppler_resolution, use_elevation=True)
